...
#DEFAULT_MIDDLE = "miles"

FILENAMES_OUTPUT = ["plot-alpha.pdf", "plot-hr.pdf", "plot-distribution-met.pdf", "plot-distribution-rv.pdf", "plot-RV-met.pdf"  ]
...
#            print("----------->0123456789012345678901234567890123456789012345678901234567890123456789           ")
            row["rv"] = float(line[12:19])
            row["teff"] = float(line[21:25])
            row["eteff"] = float(line[26:29])
            row["logg"] = float(line[30:36])
            row["elogg"] = float(line[37:41])
            row["met"] = float(line[42:48])
            row["emet"] = float(line[49:53])
            row["alp"] = float(line[54:60])
            row["ealp"] = float(line[61:65])
...
def plot_plots(finalpars):
    ...
    for s in sorted(finalpars.keys()):
        if (finalpars[s]['rv'] > -100.0) and (finalpars[s]['rv'] < 100.0):
    ...
    density_rv = gaussian_kde(rvl)
    xs_rv = np.linspace(min(rvl), max(rvl), 200)
    density.covariance_factor = lambda : .12
    density._compute_covariance()
    ax4.plot(xs_rv, density_rv(xs_rv), 'b-')
    ax4.set_xlabel('RV')
    ax4.set_ylabel('Density')
    plt.savefig(FILENAMES_OUTPUT[3])   

    f5 = plt.figure(figsize = (10,6))
    grid = gs.GridSpec(1, 2, width_ratios=[10, 0.5]) 
    ax5 = plt.subplot(grid[0])
    for s in sorted(finalpars.keys()):
        ax5.plot(finalpars[s]['met'], finalpars[s]['rv'], marker='.', color="k")
    ax5.set_xlabel('[Fe/H]')
    ax5.set_ylabel('RV')
    plt.savefig(FILENAMES_OUTPUT[4])    
   ...
   # filename_input = "avg_parameters_{}.txt".format(middle)
...
plot_plots(read_parameters_file('avg_parameters_coelho.txt') )
